...
<template>
  <DcPopup v-model:show="isShowPopup" position="bottom" lock-scroll ref="floatPopup" :overlay="false"
    class="**:overflow-x-hidden h-[70vh]">
    <DcWaterfall :source="parentComment.children" :padding="0" :col="1" :gap="0" v-if="parentComment"
      v-slot="{ item: comment }" class="h-[calc(100%-40px)]! bg-(--van-background)"
